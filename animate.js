	...
	return new Promise((fulfill, reject) => {
		...
		if (options.breakPointList === null) {
			...
			options.maxBreakPoint =
				options.breakPointList[options.breakPointList.length - 1];
		}
			...
			.then((results) => {
				Object.keys(results).forEach((key) => {
					...
					let shapes = results[key],
						config =
							typeof options.iconsConfig === 'function'
								? options.iconsConfig(key, shapes)
								: options.iconsConfig[key] === void 0
								? {}
								: options.iconsConfig[key],
						strokeCount = 0,
						fillCount = 0,
						shapeCount = 0,
						maxLength = 0,
					...
					let fillLength =
						(items.items[key].width + items.items[key].height) / 2;
					...
					shapes.forEach((shape, shapeIndex) => {
						...
						let length = config.extraLength
							? typeof config.extraLength === 'number'
								? config.extraLength
								: config.extraLength[shapeIndex] === void 0
								? 0
								: config.extraLength[shapeIndex]
							: 0;
					...
					(config.splitShortAnimations
						...
						shapes.forEach((shape, shapeIndex) => {
							...
							if (length >= config.maxBreakPoint) {
								throw new Error(
									'Shape' +
										(collection ? ' in icon ' + key : '') +
										' is longer than biggest allowed length: ' +
										length
								);
							}

							let part = (length * totalDuration) / totalLength;
							shape.animationDuration = Math.round(
								part * config.animationSegments
							);
							shape.animationDelay = Math.round(
								start * config.animationSegments
							);
							shape.lengthBreakPoint = config.breakPointList.reduce(
								(a, b) =>
									Math.min(a, b) >= length * 1.1
										? Math.min(a, b)
										: Math.max(a, b)
							);
							start += part;
						...
						Object.keys(config.copy).forEach((index) => {
							if (shapes[index] === void 0) {
								throw new Error(
									'Shape' +
										(collection ? ' in icon ' + key : '') +
										' has copy instruction for shape that does not exist: ' +
										index
								);
							}
					...
					if (config.extraDelay) {
						if (typeof config.extraDelay === 'number') {
							shapes.forEach(
								(shape) =>
									(shape.animationDelay += config.extraDelay)
							);
						} else {
							Object.keys(config.extraDelay).forEach((index) => {
								shapes[index].animationDelay +=
									config.extraDelay[index];
							});
						}
					}
